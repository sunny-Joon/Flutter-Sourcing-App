...
class _LoanEligibilityPageState extends State<LoanEligibilityPage>
                      ...
                      children: [
                        CustomPaint(
                          ...
                          painter: SemicircleProgressPainter(
                              crifScore: _animation.value.toInt()),
                        ),
                      ...
                      if (_animation.status == AnimationStatus.completed)
                        Icon(
                          crifScore > 18 && crifScore <= 650
                              ? Icons.close
                              : Icons.check,
                          color: crifScore > 18 && crifScore <= 650
                              ? Colors.red
                              : Colors.green,
                          size: 150.0,
                        ),
...
class SemicircleProgressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    drawScoreRange(
        canvas, size, Colors.green.withOpacity(0.3), 0.0, 0.02); // 0-18
    drawScoreRange(
        canvas, size, Colors.red.withOpacity(0.3), 0.02, 0.34); // 19-234
    drawScoreRange(
        canvas, size, Colors.orange.withOpacity(0.3), 0.34, 0.5); // 235-450
    drawScoreRange(
        canvas, size, Colors.yellow.withOpacity(0.3), 0.5, 0.72); // 451-650
    drawScoreRange(canvas, size, Colors.lightGreen.withOpacity(0.3), 0.72,
        0.83); // 651-750
    drawScoreRange(
        canvas, size, Colors.green.withOpacity(0.3), 0.83, 1.0); // 751-900
    ...
    textPainter('18', Offset(size.width * -0.08, size.height * .87),
        TextStyle(color: Colors.white));
    textPainter('300', Offset(size.width * 0.18, size.height * -0.02),
        TextStyle(color: Colors.white));
    textPainter('450', Offset(size.width * 0.45, size.height * -0.15),
        TextStyle(color: Colors.white));
    textPainter('650', Offset(size.width * 0.82, size.height * .1),
        TextStyle(color: Colors.white));
    textPainter('750', Offset(size.width * 0.96, size.height * .4),
        TextStyle(color: Colors.white));
    textPainter('900', Offset(size.width * 0.95, size.height * 1.05),
        TextStyle(color: Colors.white));
    textPainter(' 0 ', Offset(size.width * -0.04, size.height * 1.05),
        TextStyle(color: Colors.white));

    textPainter('Very High', Offset(size.width * 0.75, size.height * 0.8),
        TextStyle(color: Colors.white, fontWeight: FontWeight.bold));
    textPainter('High', Offset(size.width * 0.78, size.height * 0.39),
        TextStyle(color: Colors.white, fontWeight: FontWeight.bold));
    textPainter('Medium', Offset(size.width * 0.55, size.height * 0.25),
        TextStyle(color: Colors.white, fontWeight: FontWeight.bold));
    textPainter('Low', Offset(size.width * 0.35, size.height * 0.2),
        TextStyle(color: Colors.white, fontWeight: FontWeight.bold));
    textPainter('Very Low', Offset(size.width * 0.05, size.height * 0.7),
        TextStyle(color: Colors.white, fontWeight: FontWeight.bold));
  }

  void drawScoreRange(
      Canvas canvas, Size size, Color color, double start, double end) {
    ...
    final Rect arcRect =
        Rect.fromLTWH(50, 50, size.width - 100, size.height * 1.33);
  ...
  void drawNeedle(Canvas canvas, Size size, double sweepAngle) {
    ...
    final Offset center =
        Offset(size.width / 2, size.height); // Center of the circle
